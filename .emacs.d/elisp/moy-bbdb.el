...
;; The addresses matching bbdb-user-mail-address-re are never added.
...
(defun bbdb/send-hook-fetch-fields (fields)
  (when fields
    (let ((field-content (mail-fetch-field (car fields))))
      (let (tail (bbdb/send-hook-fetch-fields (cdr fields)))
        (if field-content
            (let ((elem (bbdb-extract-address-components field-content)))
              (cons elem tail))
          tail)))))

(defun bbdb/send-hook-annotate-message (rcp)
  (if (not (string-match
	    (or bbdb-user-mail-address-re
...
(defun bbdb/send-hook ()
  ...
  (let ((bbdb-notice-hook bbdb/send-notice-hook))
	...
	(if recipients
	    (let ((added-records
                   (bbdb-update-records recipients 'query)))
